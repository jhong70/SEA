...
Format of API Call: EVDB.API.call(sMethod, args, fnCallback); - sMethod is the API method to call. 
see: http://api.eventful.com/docs - args is a string or object containing the arguments to pass to the API - string: "x=1&y=2&z=3" - object: {x: 1, y: 2, z:3} - 
fnCallback is a reference to a function taking a single argument - The only argument to the function will be the data passed back from the API in JS data structures (objects, arrays, etc.). 
Example: var oArgs = { app_key: [your api key], q: "music in San Diego, CA", page_size: 25 }; 
EVDB.API.call("/events/search", oArgs, function(oData) { // Note: this relies on the custom toString() methods below alert(oData.toString()); }); Object.prototype.toString = function() { var s = "{\n"; for (var x in this) { s += "\t" + x + ": " + this[x].toString() + "\n"; } s += "}"; return s; } Array.prototype.toString = function() { return '[' + this.join(", ") + ']'; } */ 
...
EVDB.API.call = function(sMethod, args, fnCallback) { 
	try { // serialize args if necessary 
		if (typeof args == "object") { // add user-specific details to every call automatically if we have them 
		...
		} else { // add user-specific details to every call automatically if we have them 
		...
		elScript.src = EVDB.API.URL + sMethod +'?'+ args +"&json_request_id="+ EVDB.API.idRequest; // add the call to the requests queue 
		EVDB.API.requests[EVDB.API.idRequest++] = {el: elScript, cb: fnCallback}; // append the script element to the dom (to fetch the data) 
		if (!EVDB.API.elHead) { 
...
EVDB.API._complete = function(idRequest, oData) { 
	...
	if (requestData) { // if we have a valid callback 
		if (typeof requestData.cb == "function") { 
			var fnCallback = requestData.cb; // clean up the request 
			try { 
				EVDB.API.elHead.removeChild(requestData.el); 
			} catch (e) {} 
			EVDB.API.requests[idRequest] = null; // fire the callback with the data 
			fnCallback(oData); 
			return true; 
		} else { 
